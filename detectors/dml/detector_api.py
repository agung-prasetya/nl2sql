...
@app.route('/dml/detect', methods=['POST'])
def predict_dml():
...
@app.route('/dml/evaluate', methods=['POST'])
def evaluate():
    filepath_dataset = str(Path(__file__).parent/"dataset/dataset.xlsx")
    folderpath_database = str(Path(__file__).parent/"dataset/")
    evaluator = SingleLabelEvaluator(filepath_dataset=filepath_dataset,folderpath_database=folderpath_database, detector=DMLDetector())
