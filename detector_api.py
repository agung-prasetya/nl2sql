...
def evaluate():
    filepath_dataset = str(Path(__file__).parent / "detectors\sketch\dataset\dataset.xlsx")
    folderpath_database = str(Path(__file__).parent / "detectors/sketch/dataset/")
    evaluator = SingleLabelEvaluator(filepath_dataset=filepath_dataset, folderpath_database=folderpath_database, detector=SketchDetector())
